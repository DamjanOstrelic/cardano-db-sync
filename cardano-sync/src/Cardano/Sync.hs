    ...
    orDie renderSyncNodeError $ do
      ...
      case genCfg of
          GenesisCardano _ bCfg _sCfg -> do
            ...
            let codecConfig = cardanoCodecConfig bCfg
            let socketPath = enpSocketPath enp
            liftIO $ runSyncNodeNodeClient prometheusPort syncEnv
                iomgr trce plugin runDBThreadFunction codecConfig socketPath
  where
